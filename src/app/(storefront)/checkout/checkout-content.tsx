...
// Shipping methods themselves are admin-managed (see /admin/shipping-methods)
// — this just maps the checkout's local region type to the DB's uppercase
// DeliveryRegion enum keys the methods prop is grouped by.
function regionKey(location: DeliveryRegion): "PHNOM_PENH" | "PROVINCE" {
  return location === LOCATION_PHNOM_PENH ? "PHNOM_PENH" : "PROVINCE";
}
...
interface CheckoutContentProps {
  shippingMethods: Record<"PHNOM_PENH" | "PROVINCE", ShippingMethodDTO[]>;
}

export function CheckoutContent({ shippingMethods }: CheckoutContentProps) {
  const { t } = useLanguage();
  ...
  // Shipping & payment — null when the region's courier list (admin-managed)
  // is empty, which the UI has to treat as a real, if unlikely, state.
  const [selectedShipping, setSelectedShipping] = useState<ShippingMethodDTO | null>(
    shippingMethods[regionKey(LOCATION_PHNOM_PENH)][0] ?? null
  );
  ...
  const handleLocationChange = (loc: DeliveryRegion) => {
    setLocation(loc);
    setSelectedShipping(shippingMethods[regionKey(loc)][0] ?? null);
    setSelectedProvince("");
    setSelectedDistrict("");
    setStepError(null);
  };
  ...
  const total = subtotal + (selectedShipping?.cost ?? 0) - (appliedCoupon?.discountAmount ?? 0);
  ...
  const handleNextFromDelivery = () => {
    ...
    if (!selectedShipping) {
      setStepError(t("checkout.errorNoShipping"));
      return;
    }
    goToStep(2);
  };
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    // Guarded by handleNextFromDelivery before step 2 is reachable — this is
    // just to satisfy the type checker, not a real runtime path.
    if (!selectedShipping) {
      setStepError(t("checkout.errorNoShipping"));
      return;
    }
              ...
              >
                ...
                {((location === LOCATION_PHNOM_PENH && (!!coords || address.trim().length > 0)) ||
                  ...
                  >
                    ...
                    {shippingMethods[regionKey(location)].length > 0 ? (
                      <div className="flex flex-col gap-2.5">
                        {shippingMethods[regionKey(location)].map((method) => (
                          <div
                            key={method.id}
                      ...
                      <p className="text-sm text-muted-foreground">{t("checkout.errorNoShipping")}</p>
                    )}
                  </motion.div>
                )}
              ...
              {selectedShipping && (
                <div className="flex justify-between items-baseline gap-2 text-muted-foreground">
                  <span className="truncate">
                    {t("checkout.deliveryVia", { name: selectedShipping.name })}
                  </span>
